    ...
    <div className="plugin-inventory">
      <div className="plugin-inventory-tools">
        <label className="plugin-search">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索名称、模块名或 Loader ID" />
      ...
      <div className="plugin-inventory-grid">
        ...
        {visible.length === 0 && <div className="plugin-empty"><MagnifyingGlass size={27} weight="duotone" /><strong>没有匹配的插件</strong><span>调整搜索词或筛选条件后再试。</span></div>}
      </div>
    </div>
  );
}
...
export function PluginSettingsPanel({ desktop }) {
  ...
  useEffect(() => {
    ...
    return globalThis.window.jimu.plugins.subscribe((update) => {
